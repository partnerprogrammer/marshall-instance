...
describe('add-thread-nudge wiring in src/modules/index.ts', () => {
  it("imports './thread-nudge/index.js' as a top-level statement, after the mailbox import", () => {
    ...
    expect(nudgeIdx, "import './thread-nudge/index.js' must be a top-level statement of the barrel").toBeGreaterThanOrEqual(0);
    expect(nudgeIdx, 'the thread-nudge import must come after the mailbox import (documented barrel invariant)').toBeGreaterThan(
      mailboxIdx
    );
  });
});
