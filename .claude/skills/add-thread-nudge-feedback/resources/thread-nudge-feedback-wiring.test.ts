...
describe('add-thread-nudge-feedback wiring in src/modules/index.ts', () => {
  it("imports './thread-nudge-feedback/index.js' as a top-level statement, after its thread-nudge dependency", () => {
    ...
    expect(nudgeIdx, "thread-nudge's own import not found — add-thread-nudge-feedback requires add-thread-nudge").toBeGreaterThanOrEqual(0);
    expect(feedbackIdx, "import './thread-nudge-feedback/index.js' must be a top-level statement of the barrel").toBeGreaterThanOrEqual(0);
    expect(feedbackIdx, 'thread-nudge-feedback must be imported after thread-nudge (real dependency: config.ts imports from ../thread-nudge/config.js)').toBeGreaterThan(
      nudgeIdx
    );
  });
});
