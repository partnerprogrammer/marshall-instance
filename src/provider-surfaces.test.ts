...
describe('derived provider spawn surfaces', () => {
  ...
  it('refuses a prepared-file symlink instead of creating its target outside the state volume', async () => {
    const sessionDirectory = path.join(TEST_ROOT, 'symlink-file-session');
    const state = path.join(sessionDirectory, '.state');
    const escaped = path.join(sessionDirectory, 'escaped-created');
    fs.mkdirSync(state, { recursive: true });
    fs.symlinkSync('../escaped-created', path.join(state, 'auth.json'));
    const contract: ProviderHostContract = {
      ...backingContract(),
      stateVolumes: [
        {
    ...
    await expect(
      realizeProviderSpawnSurfaces('symlink-file', contract, 'group', GROUPS_DIR, sessionDirectory, [], {
        legacyOverlay: async () => ({}),
        composeProjectDocument: async () => {},
      }),
  ...
  it('refuses a symlinked skill directory instead of synchronizing outside the group', async () => {
    const groupDir = path.join(TEST_ROOT, 'symlink-backing-group');
    const escaped = path.join(TEST_ROOT, 'escaped-skills');
    fs.mkdirSync(path.join(groupDir, '.agents'), { recursive: true });
    fs.mkdirSync(escaped);
    fs.symlinkSync('../../escaped-skills', path.join(groupDir, '.agents', 'skills'));
    const contract = backingContract({
      id: 'skills',
      location: { kind: 'group-directory', directory: '.agents', subdirectory: '' },
      skillsSubdirectory: 'skills',
    ...
    await expect(
      realizeProviderSpawnSurfaces('symlink-skills', contract, 'group', groupDir, DATA_DIR, ['welcome'], {
        legacyOverlay: async () => ({}),
        composeProjectDocument: async () => {},
      }),
    ).rejects.toThrow(/symbolic link/);
    expect(fs.existsSync(path.join(escaped, 'welcome'))).toBe(false);
  });
