 ...
 * always produced. The spec is cast at the call site because the two core
 * generations type `ProjectDocSpec` differently.
 *
...
export function composeGroupAgentsMd(group: AgentGroup, groupDir: string): Promise<void> {
  ...
  // them (and never call this path for surfaces). One cast keeps the payload
  // compiling against both generations.
  return composeGroupProjectDoc(group, groupDir, CODEX_PROJECT_DOC as unknown as ProjectDocSpec);
}
